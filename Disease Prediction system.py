...
# Loading the models
diabetes_model = pickle.load(open('C:/Users/ASUS/Desktop/last year Disease Prediction/Disease Saved Models/diabetes_model.sav', 'rb'))
heart_disease_model = pickle.load(open('C:/Users/ASUS/Desktop/last year Disease Prediction/Disease Saved Models/heart_disease_model.sav', 'rb'))
kidney_model = pickle.load(open('C:/Users/ASUS/Desktop/last year Disease Prediction/Disease Saved Models/kidney_model.pkl', 'rb'))
breast_cancer_model = pickle.load(open('C:/Users/ASUS/Desktop/last year Disease Prediction/Disease Saved Models/breast_cancer_model.sav', 'rb'))
...
def heart_disease_prediction_page():
    ...
    with col1:
        thal = st.text_input('thal: 0 = normal; 1 = fixed defect; 2 = reversable defect(thal)')
...
def kidney_disease_prediction_page():
    ...
    with col3:
        specific_gravity = st.text_input('Specific Gravity(sg)')

    with col4:
        albumin = st.text_input('Albumin(al)')

    with col5:
        sugar = st.text_input('Sugar(su)')

    with col1:
        red_blood_cells = st.text_input('Red Blood Cell (normal=0 and if not then 1)(rbc)')

    with col2:
        pus_cell = st.text_input('Pus Cell (normal=0 and if not then 1)(pc)')

    with col3:
        pus_cell_clumps = st.text_input('Pus Cell Clumps (not present=0 and if yes then 1)(pcc)')

    with col4:
        bacteria = st.text_input('Bacteria (not present=0 and if yes then 1)(ba)')

    with col5:
        blood_glucose_random = st.text_input('Blood Glucose Random(bgr)')

    with col1:
        blood_urea = st.text_input('Blood Urea(bu)')

    with col2:
        serum_creatinine = st.text_input('Serum Creatinine(sc)')

    with col3:
        sodium = st.text_input('Sodium(sod)')

    with col4:
        potassium = st.text_input('Potassium(pot)')

    with col5:
        haemoglobin = st.text_input('Haemoglobin(haemo)')

    with col1:
        packed_cell_volume = st.text_input('Packet Cell Volume(pcv)')

    with col2:
        white_blood_cell_count = st.text_input('White Blood Cell Count(wc)')

    with col3:
        red_blood_cell_count = st.text_input('Red Blood Cell Count(rc)')

    with col4:
        hypertension = st.text_input('Hypertension (yes=1 and no=0)(htn)')

    with col5:
        diabetes_mellitus = st.text_input('Diabetes Mellitus (yes=1 and no=0)(dm)')

    with col1:
        coronary_artery_disease = st.text_input('Coronary Artery Disease (yes=1 and no=0)(cad)')

    with col2:
        appetite = st.text_input('Appetitte (good=1 and bad=0)(appet)')

    with col3:
        peda_edema = st.text_input('Peda Edema (yes=1 and no=0)(pe)')

    with col4:
        aanemia = st.text_input('Aanemia (yes=1 and no=0)(ane)')
